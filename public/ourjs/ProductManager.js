...
async function ProductManager(tabName){
    ...
    try {
      ...
      if (Apidata["status"] === "success") {
        let cartStatus = priviewCartstatus(Apidata.data.detailed.rejected_carts)
...
export async function getPMData(data){
    const WholeMainContent = document.getElementById('WholeMainContent')
    if(data == 'Raise Cart'){
   ...
   <thead>
    <tr role="row">
     ...
     <th class="text-center sorting_disabled" rowspan="1" colspan="1" >View</th>
        ...
        let ProductmanagerData = await ProductManager("ProductManager")
        console.log(ProductmanagerData,"ProductmanagerData")
        const cartTraker = document.getElementById("cartTraker")
        if(ProductmanagerData){
            ...
            tabs.forEach((item, index)=>{
             item.addEventListener("click", () => {
                ...
                console.log("ggggghhhjjjkkkk",ProductmanagerData.data.detailed[sumArr[index]],sumArr[index])
                let cartSpec = document.getElementById('cartSpec')
                console.log("getcartspec", cartSpec)
                if(cartSpec){
                 cartSpec.remove()
                }
                let cartStatus = priviewCartstatus(ProductmanagerData.data.detailed[sumArr[index]])
                let cartBody = document.getElementById("cartStatusBody")
                cartBody.innerHTML= cartStatus;
            });
            })
...
window.viewSpecificCartPm = function (gotdata, index) {
    ...
    statusBtn.forEach((btn) => {
        ...
        if (btn.classList.contains('text-success')) {
            ...
            btn.classList.add("bg-light-warning", "border-light-warning", "text-warning");
        }
    });
    ...
    let pageBody = document.querySelector(".appendCart");
    let data = Array.isArray(gotdata.items) ? gotdata.items : [];
    ...
    let tableBodyRows = data.map((item, rowIndex) => {
        return `<tr>${tableHeaders
                ...
                if (key === "qty" && gotdata.cartstage === "S") {
                    return `
                        <td>
                            <fieldset class="qty-box">
                                <div class="input-group bootstrap-touchspin">
                                    <button class="btn btn-primary btn-square bootstrap-touchspin-down cartPageMinusPm1" data-index="${rowIndex}" type="button">
                                        <i class="fa-solid fa-minus"></i>
                                    </button>
                                    <input class="touchspin text-center form-control cartPageInputPm1" data-index="${rowIndex}" type="text" value="${item[key] ?? 1}" style="display: block;">
                                    <button class="btn btn-primary btn-square bootstrap-touchspin-up cartPagePlusPm1" data-index="${rowIndex}" type="button">
            ...
            .join("")}
            ${gotdata.cartstage === "S" ? 
            `<td><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x-circle removeProductCartPage" data-index="${rowIndex}">
    ...
    let cartTable = `
        <table class="table table-bordered">
            <thead class="theadCart">
                <tr>${tableHeaders.map((header) => `<th>${header}</th>`).join("")}${gotdata.cartstage === "S" ? `<th>Action</th>` : ""}</tr>
            </thead>
            <tbody id="specificCartBodyPm">
                ${tableBodyRows.join("")}
                ${gotdata.cartstage === "S" ? 
                `<tr>
                    <td class="text-end" colspan="8">
                        <a class="btn btn-secondary cart-btn-transform" id="rejectButton">Reject</a>
                        <a style="margin-left:20px" class="btn btn-success cart-btn-transform" id="approveButton">Approve</a>
                    </td>
                </tr>` : ""}
            </tbody>
        </table>`;
    ...
    pageBody.innerHTML = `
        <div class="container-fluid" id="cartSpec">
            <div class="row">
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-header card-no-border pb-0">
    ...
    document.querySelectorAll(".cartPageMinusPm1").forEach((btn) => {
        btn.addEventListener("click", function () {
            ...
            if (currentValue > 1) {
                ...
                row.remove(); // Remove row when quantity reaches zero
            }
        });
    });

    document.querySelectorAll(".removeProductCartPage").forEach((btn) => {
        btn.addEventListener("click", function () {
            let index = this.getAttribute("data-index");
            let row = this.closest("tr");
                row.remove(); 
    ...
    document.querySelectorAll(".cartPagePlusPm1").forEach((btn) => {
        btn.addEventListener("click", function () {
            let index = this.getAttribute("data-index");
            let inputField = document.querySelector(`.cartPageInputPm1[data-index="${index}"]`);
            let currentValue = parseInt(inputField.value) || 1;
...
function getUpdatedCartData() {
    ...
    document.querySelectorAll("#specificCartBodyPm tr").forEach((row) => {
        let rowIndex = row.querySelector(".cartPageInputPm1")?.getAttribute("data-index");
        if (rowIndex !== null) {
            let inputs = row.querySelectorAll("td"); 
            let itemData = {};

            inputs.forEach((td, i) => {
                let input = td.querySelector("input.cartPageInputPm1");
                let key = document.querySelector(".theadCart tr").children[i]?.textContent.trim(); // Get column header name

                if (input) {
                    itemData[key] = parseInt(input.value); // Store updated quantity
                } else {
                    itemData[key] = td.textContent.trim(); // Store other fields
                }
            });

            updatedCart.push(itemData);
        }
    });
...
function priviewCartstatus(Cartdata) {
    ...
    let cartBody = Cartdata.map((data, index) => {
        return `<tr role="row" class="odd">
                    ...
                    <td class="text-end">
                        <div 
                            onClick='viewSpecificCartPm(${JSON.stringify(data)}, ${index})' 
                            class="btn bg-light-warning border-light-warning text-warning viewSpecificCartPm">
                            View Cart
                        </div>
                    </td>
                </tr>`;
